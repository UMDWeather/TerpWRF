...
#------------------------------------------------------------------------------------------------
def writeIOcontrol(dest):
    ''' Copy the specified field list files in the namelist template for
	surface / other special wrfout streams '''
    log=getFnLog()
    iofiles = ['io_fields_d01.txt','io_fields_d02.txt']
    for iofile in iofiles:
	shutil.copy(configFiles+'/'+iofile,dest)
